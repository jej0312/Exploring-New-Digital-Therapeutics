...
class Graph():
    ...
    def node2vec_walk(self, walk_length, start_node, G):
        ...
        while len(walk) < walk_length:
            ...
            cur_nbrs = G[cur] # neighbors보다 indexing이 더 빠름 https://frhyme.github.io/python-libs/nx_AtlasView_is_faster_than_neighbor/
            # print(cur_nbrs)
            if len(list(cur_nbrs)) > 0:
                if len(walk) == 1:
                    walk.append(list(cur_nbrs)[alias_draw(alias_nodes_list[G][cur][0], alias_nodes_list[G][cur][1])])
                else:
                    prev = walk[-2]
                    if (prev, cur) not in alias_edges_list[G].keys():
                        prev = list(cur_nbrs)[alias_draw(alias_nodes_list[G][cur][0], alias_nodes_list[G][cur][1])]
                    next = list(cur_nbrs)[
