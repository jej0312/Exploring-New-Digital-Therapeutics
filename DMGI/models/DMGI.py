    ...
    def training(self):
        ...
        for epoch in range(self.args.nb_epochs):
            ...
            if loss < best:
                best = loss
                cnt_wait = 0
                torch.save(model.state_dict(), 'revision/best_{}_{}_{}_{}_{}.pkl'.format(self.args.dataset, self.args.nb_epochs, self.args.hid_units, self.args.lr, self.args.isAttn))
            else:
                cnt_wait += 1
        ...
        model.load_state_dict(torch.load('revision/best_{}_{}_{}_{}_{}.pkl'.format(self.args.dataset, self.args.nb_epochs, self.args.hid_units, self.args.lr, self.args.isAttn)))

        # Evaluation
        model.eval()
        # evaluate(model.H.data.detach(), self.idx_train, self.idx_val, self.idx_test, self.labels, self.args.device)

        with open('revision/best_{}_{}_{}_{}_{}_model.pkl'.format(self.args.dataset, self.args.nb_epochs, self.args.hid_units, self.args.lr, self.args.isAttn), 'wb') as f:
            pkl.dump(model, f)
