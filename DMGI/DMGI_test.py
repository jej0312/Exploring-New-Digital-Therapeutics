...
if (flag == 'preprocessing') | (flag == 'all'):
    ...
    if number_of_groups == 0:
        ...
        for i in range(number_of_groups):
            ...
            with open(f'DMGI/data/{dataset}_{weighted}_{i + 1}_{number_of_groups}.pkl', 'wb') as f:
                pickle.dump(data, f)
...
if (flag == 'link prediction') | (flag == 'all') | (flag == 'modeling'):
    ...
    def plot_roc_curve(y_true, y_scores, model_type = 'DMGI'):
        global i, iterat
        import matplotlib.pyplot as plt
        cmap = {'0': 'red', '1': 'brown', '2': 'green', '3': 'blue', '4': 'grey'}

        import os.path
        import pickle

        if os.path.exists(os.path.join('revision', f'time{iterat}_{model_type}_{weighted}_{nb_epochs}_{hid_units}_{lr}_{isAttn}_model.pickle')):
            with open(os.path.join('revision', f'time{iterat}_{model_type}_{weighted}_{nb_epochs}_{hid_units}_{lr}_{isAttn}_model.pickle'), 'rb') as f:
                (fig, ax) = pickle.load(f)
        else:
            fig, ax = plt.subplots(1, 1)

        fper, tper, thresholds = roc_curve(y_true, y_scores)
        ax.plot(fper, tper, color=cmap[str(i)], label=f'{model_type} fold {i+1}')
        ...
        pickle.dump((fig, ax), open(os.path.join('revision', f'time{iterat}_{model_type}_{weighted}_{nb_epochs}_{hid_units}_{lr}_{isAttn}_model.pickle'), 'wb'))
        fig.savefig(os.path.join('revision', f'time{iterat}_{model_type}_{weighted}_{nb_epochs}_{hid_units}_{lr}_{isAttn}_model-ROC.png'), dpi=300)
    ...
    def calculate_performance(y_true, y_pred, cutoff=0.45):
        auc = roc_auc_score(y_true, y_pred)
        # auc = roc_auc_score(y_true, y_pred >= cutoff)
        plot_roc_curve(y_true, y_pred >= cutoff)
        acc = accuracy_score(y_true, y_pred >= cutoff)
        rec = recall_score(y_true, y_pred >= cutoff)
        prec = precision_score(y_true, y_pred >= cutoff)
        f1 = f1_score(y_true, y_pred >= cutoff)
        return auc, acc, rec, prec, f1
    ...
    def get_dict_AUC(model, true_edges, false_edges):
        ...
        return calculate_performance(y_true, y_scores)
    ...
    def get_AUC_from_edges(edges):
        ...
        tmp_DMGI_auc, tmp_DMGI_acc, tmp_DMGI_rec, tmp_DMGI_prec, tmp_DMGI_f1 = get_dict_AUC(dmgi_model, true_edges, selected_false_edges)

        return tmp_DMGI_auc, tmp_DMGI_acc, tmp_DMGI_rec, tmp_DMGI_prec, tmp_DMGI_f1

    overall_DMGI_performance = list()

    # train-test-valid
    if number_of_groups == 0:
        with open('saved_model/best_{}_{}_{}_{}_{}_{}_model.pkl'.format(dataset, weighted, nb_epochs, hid_units, lr, isAttn), 'rb') as f:
            model = pickle.load(f)
        model.load_state_dict(torch.load('DMGI/saved_model/best_{}_{}_{}_{}_{}_{}.pkl'.format(dataset, weighted, nb_epochs, hid_units, lr, isAttn)))
        ...
        overall_DMGI_auc, overall_DMGI_acc, overall_DMGI_rec, overall_DMGI_prec, overall_DMGI_f1 = get_AUC_from_edges(edges)

    # k-fold
    else:

        with open(os.path.join('revision', 'overall_performance-revision' '.txt'), 'a') as f:
            f.write(
                f"\n================ model: {dataset}_{weighted}_{nb_epochs}_{hid_units}_{lr}_{isAttn} ==================\n")

        tmp_DMGI_performance = 0

        for iterat in range(iteration):

            overall_DMGI_performance = defaultdict(list)

            for i in range(number_of_groups):
                print(f'Working on {dataset}_{weighted}_iter{iterat}_fold{i + 1}_{number_of_groups}_{isAttn}.pkl')

                with open(
                        f'saved_model/best_{dataset}_{weighted}_{i + 1}_{number_of_groups}_{nb_epochs}_{hid_units}_{lr}_{isAttn}_model.pkl',
                        'rb') as f:
                    model = pickle.load(f)
                model.load_state_dict(torch.load(
                    f'saved_model/best_{dataset}_{weighted}_{i + 1}_{number_of_groups}_{nb_epochs}_{hid_units}_{lr}_{isAttn}.pkl'))
                dmgi_model = {str(j): model.H.detach().reshape((model.H.detach().shape[1], -1))[j] for j in
                              range(model.H.detach().shape[1])}

                with open(f'data/{dataset}_{weighted}_{i + 1}_{number_of_groups}.pkl', 'rb') as f:
                    edges = pickle.load(f)

                tmp_DMGI_auc, tmp_DMGI_acc, tmp_DMGI_rec, tmp_DMGI_prec, tmp_DMGI_f1 = get_AUC_from_edges(edges)
                print('DMGI score:', tmp_DMGI_auc)


                overall_DMGI_performance['auc'].append(tmp_DMGI_auc)
                overall_DMGI_performance['accuracy'].append(tmp_DMGI_acc)
                overall_DMGI_performance['precision'].append(tmp_DMGI_rec)
                overall_DMGI_performance['recall'].append(tmp_DMGI_prec)
                overall_DMGI_performance['f1_score'].append(tmp_DMGI_f1)
            ...
            with open(os.path.join('revision', 'overall_performance-revision' '.txt'), 'a') as f:
                f.write("================ iterations: " + str(iterat) + " ==================\n")
                f.write("DMGI: " + str(overall_DMGI_performance.reshape(1)[0].items()) + "\n")
                f.write(f'iteration {iterat} -- Overall DMGI performance:' + str({k: np.mean(v) for k, v in overall_DMGI_performance.reshape(1)[0].items()}) + "\n")
                f.write(f'iteration {iterat} -- Overall DMGI std:' + str({k: np.std(v) for k, v in overall_DMGI_performance.reshape(1)[0].items()}) + "\n")

            print(f'iteration {iterat} -- Overall DMGI performance:', {k: np.mean(v) for k,v in overall_DMGI_performance.reshape(1)[0].items()})
            print(f'iteration {iterat} -- Overall DMGI std:', {k: np.std(v) for k, v in overall_DMGI_performance.reshape(1)[0].items()})
